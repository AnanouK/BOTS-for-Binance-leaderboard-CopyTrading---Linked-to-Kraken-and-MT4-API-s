    ...
    try {
        ...
        .then(result => {
            result.result.map(name => {
                if(name.future === namecurrency+"-PERP"){
                    exist = name.openSize;
                }
            })
          })

        if(exist == 0 && reduceOnly)
        {
        ...
        .then(result => {
            result.result.map(name => {
                if(name.future === namecurrency+"-PERP"){
                    ...
                    let ordersize = 0;
                    client.setAccountLeverage(2).catch((error) => {console.error("erreur dans le set leverage " + error);});
                    client.getPositions()
                    .then(result => {
                        result.result.map(name => {
                            if(name.future === namecurrency+"-PERP"){
                                ordersize = name.openSize;
                            }
                        })
                      })
